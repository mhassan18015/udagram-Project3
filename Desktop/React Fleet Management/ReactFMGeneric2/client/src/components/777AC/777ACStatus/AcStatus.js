...
const AcStatus = () => {
  ...
  const [lastInputDataGE90, setlastInputDataGE90] = useState(
    JSON.parse(localStorage.getItem('lastInputDataGE90')) || {}
  )

  useEffect(() => {
    axios
      .get('https://fleetmanagement-api.onrender.com/AcStatusGE90')
      .then((response) => {
  ...
  useEffect(() => {
    // Set lastInputDataGE90 to a minimum value of 11000 for CSN < 11000
    const updatedData = { ...lastInputDataGE90 }
    acStatusData.forEach((item) => {
      if (item.CSN < 3500) {
        updatedData[item.AcEngPos] = 3400
      }
    })

    // Only update lastInputDataGE90 if it's not already initialized
    if (Object.keys(lastInputDataGE90).length === 0) {
      setlastInputDataGE90(updatedData)
    }

    // Save the updated lastInputDataGE90 to local storage
    localStorage.setItem('lastInputDataGE90', JSON.stringify(updatedData))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [acStatusData])

  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem('lastInputDataGE90'))

    if (!storedData || Object.keys(storedData).length === 0) {
      // Initialize lastInputDataGE90 with a default value of 11000 for CSN < 11000
      const updatedData = {}
      acStatusData.forEach((item) => {
        if (item.CSN < 3400) {
          updatedData[item.AcEngPos] = 3400
      ...
      setlastInputDataGE90(storedData)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])
  ...
  const handleSort = (key) => {
    ...
    sortedData.sort((a, b) => {
      if (key === 'MinimumValue') {
        ...
        const aEGTM = parseInt(a.EGTMGE90s[0]?.EGTM || 0)
        const bEGTM = parseInt(b.EGTMGE90s[0]?.EGTM || 0)
  ...
  const calculatedValues = (item) => {
    const lastInputValueExpectedCoreLife =
      lastInputDataGE90[item.AcEngPos] || ''
    // const CSOcalculated = item?.CSO - item.engine_sv?.C_at_LSV
    const DataDate = item.GeneralGE90?.DataDate
    const Date_Of_Validity = DataDate ? new Date(DataDate) : null // Creating Date object if DataDate is defined, else assigning null
    const Average_CyclePerDay = item.rotableutilizationGE90?.AverCycPerDay
    const numberOfDaysToAdd =
      (lastInputValueExpectedCoreLife - item?.CSO) / Average_CyclePerDay
    // const numberOfDaysToAdd =
    //   (lastInputValueExpectedCoreLife -
    //     (item?.CSN - item.engine_svGE90?.C_at_LSV)) /
    //   Average_CyclePerDay
    ...
    const numberOfDaysToAddLLP = item.remainingGE90?.Togo / Average_CyclePerDay
    ...
    if (item?.CSO > 1000) {
      numberOfDaysToAddEGTM =
        ((item.EGTMGE90s[0]?.EGTM / 1.2) * 200) / Average_CyclePerDay
    } else {
      numberOfDaysToAddEGTM =
        ((item.EGTMGE90s[0]?.EGTM / 2.4) * 200) / Average_CyclePerDay
    }
          ...
          <tbody>
            {acStatusData.map((item, index) => {
              if (item.Status === 'Installed') {
                const lastInputValueExpectedCoreLife =
                  lastInputDataGE90[item.AcEngPos] || 3400
                ...
                return (
                  <tr key={index} className={rowClass}>
                    ...
                    <td>
                      <a
                        href={`https://fleetmanagement-api.onrender.com/engine-dataGE90/bySN/${item.Eng}`} //
                    ...
                    <td>{item.engine_svGE90?.C_at_LSV || '-'}</td>
                    <td>{item?.TSO || '-'}</td>
                    <td>{item?.CSO || '-'}</td>
                    <td>{item?.CSN - item.engine_svGE90?.C_at_LSV}</td>
                    {/* <td>{item?.TSI || '-'}</td>
                    <td>{item?.CSI || '-'}</td> */}
                    <td>{item.EGTMGE90s[0] ? item.EGTMGE90s[0].EGTM : '-'}</td>
                    <td>{item.rotableutilizationGE90?.AverCycPerDay || '-'}</td>
                    <td>{item.LLPGE90s[0]?.CycleLimit - item?.CSN || '-'}</td>
                    <td>{item.remainingGE90?.Togo || '-'}</td>
                    {/* <td>{item.LLPs[0]?.CRemainingCalc || '-'}</td> */}
                    <td>{item.LLPGE90s[0]?.CycleLimit || '-'}</td>
                    {/* <td>{item.LLPs[0]?.Description || '-'}</td> */}
                    <td>
                      <a
                        href={`https://fleetmanagement-api.onrender.com/llpGE90/byEng/${item.Eng}`} //
                        target='_blank' // Optional: Opens the link in a new tab
                        rel='noopener noreferrer' // Recommended when using target="_blank"
                      >
                        {item.LLPGE90s[0]?.Description}
                      </a>
                    </td>
                    <td>
                      <input
